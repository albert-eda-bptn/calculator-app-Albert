        ...
        switch(input.type){
          ...
          case InputType.Operator:
            if (input.operator === OperatorType.Equals){
              ...
              return {
                operations: [...builder.operations, builder.working], working: {operator: input.operator, value:0},
              };
            }
  ...
  operations.reduce<number>((sum, operation) => {
      switch(operation.operator){
        ...
        case OperatorType.Squareroot:
          return sum + Math.sqrt(operation.value) ; 
        case OperatorType.Percentage:
          return (sum + operation.value)/100; 
        case OperatorType.Decimal:
          return sum + Number((operation.value/10).toFixed(2)); 
